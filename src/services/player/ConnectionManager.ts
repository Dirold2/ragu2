...
export class ConnectionManager extends EventEmitter {
  ...
  constructor(
    ...
    private readonly client: ClientSubset,
  ) {
    super();
  }
    ...
    const statusCheckInterval = setInterval(async () => {
      ...
      if (
        ...
        try {
          ...
          this.logger?.debug?.(
            `[ConnectionManager] Error during connection.destroy in statusCheckInterval: ${e instanceof Error ? e.message : String(e)}`,
          );
        }
      }
    }, 10_000);
