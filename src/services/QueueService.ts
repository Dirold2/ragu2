...
export default class QueueService {
	...
	public async setTrack(
		...
		try {
			...
			this.handleError(
				error,
				bot.loggerMessages.ERROR_ADDING_TRACK_TO_QUEUE(track.trackId),
			);
		}
	}
	...
	public async getTrack(guildId: string): Promise<Track | null> {
		if (!guildId)
			throw new QueueServiceError(bot.loggerMessages.GUILD_ID_IS_REQUIRED);
		try {
			...
			this.handleError(
				error,
				bot.loggerMessages.ERROR_RETRIEVING_TRACK_FROM_QUEUE(guildId),
			);
			return null;
		}
	}
	...
	public async getQueue(guildId: string): Promise<QueueResult> {
		try {
			...
			this.handleError(
				error,
				bot.loggerMessages.ERROR_FETCHING_QUEUE_FOR_GUILD(guildId),
			);
	...
	public async clearQueue(guildId: string): Promise<void> {
		try {
			...
			this.handleError(
				error,
				bot.loggerMessages.ERROR_CLEARING_QUEUE_FOR_GUILD(guildId),
			);
		}
	}
	...
	public async removeTrack(guildId: string, trackId: string): Promise<void> {
		...
		try {
			...
			this.handleError(
				error,
				bot.loggerMessages.ERROR_REMOVING_TRACK_FROM_QUEUE(trackId),
			);
		}
	}
	...
	public async logTrackPlay(
		...
		try {
			...
			this.handleError(
				error,
				bot.loggerMessages.ERROR_LOGGING_PLAY_FOR_TRACK(trackId, userId),
			);
		}
	}
	...
	public async getLastPlayedTracks(
		...
		try {
			...
			this.handleError(
				error,
				bot.loggerMessages.ERROR_FETCHING_LAST_PLAYED_TRACKS_FOR_USER(userId),
			);
			return [];
		}
	}
	...
	public async getTopPlayedTracks(limit: number = 10): Promise<Track[]> {
		try {
			...
			this.handleError(
				error,
				bot.loggerMessages.ERROR_FETCHING_TOP_PLAYED_TRACKS,
			);
			return [];
		}
	}
	...
	public async setTracks(
		...
		if (!tracks.length)
			throw new QueueServiceError(
				bot.loggerMessages.TRACKS_ARRAY_CANNOT_BE_EMPTY,
			);
		...
		try {
			...
			logger.info(
				`${bot.loggerMessages.ADDED_BATCH_OF_TRACKS_GUILD(newTracks.length, guildId)}`,
			);
		} catch (error) {
			this.handleError(error, bot.loggerMessages.ERROR_ADDING_TRACKS_TO_QUEUE);
		}
	}
	...
	private validateParams(
		...
		if (
			...
			throw new QueueServiceError(
				bot.loggerMessages.MISSING_REQUIRED_PARAMETERS,
			);
		}
	}
	...
	private async addTrackBatch(tracks: Track[], queueId: number): Promise<void> {
		...
		logger.info(`${bot.loggerMessages.ADDED_BATCH_OF_TRACKS(tracks.length)}`);
	}
}
