 ...
 */
export async function loadTranslation(moduleName: string, language: string): Promise<any | null> {
  // Try cache first
...
async function findLocalesPath(moduleName: string): Promise<string | null> {
  ...
  const moduleLocalesPath = PathResolver.getSrcRelativePath("modules", moduleName, "locales");
